        ...
        <div className="relative h-80 w-full overflow-hidden rounded-2xl bg-black">
          ...
          >
            <button
              ...
              className="flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg backdrop-blur transition hover:scale-105"
            >
              {isPlaying ? (
                
                <div className="flex gap-1">
                  <span className="h-5 w-1.5 bg-black" />
                  <span className="h-5 w-1.5 bg-black" />
                </div>
              ) : (
        ...
        <div className="mt-4">
          ...
          <div className="my-2 flex flex-wrap gap-2">
            {tags.map((tag) => (
              <span
                key={tag}
                className={`rounded-full px-3 py-1 text-xs ${tagStyle}`}
              >
                {tag}
              </span>
            ))}
      ...
      <div className="mx-auto max-w-7xl px-6">
        ...
        <div className="grid grid-cols-1 gap-x-6 gap-y-40 px-4 md:grid-cols-12">
          ...
          <FacilityCard
            ...
            videoSrc="/images/homepagevid.mp4"
          />
